...
All changes pushed to a UI project's main branch can trigger a new release (not described here).
...
Use the following command to create a local development branch named `name-me`:
 ...
 $ git checkout -b name-me -t origin/main
...
As soon as the pull request is merged into main, an automated process will take over to publish a new release for the site generator to use.
